document.addEventListener('DOMContentLoaded', function () {
    ...
    $("#menuId").val($(parent.document).find("#editMenuId").val());
    let url = "/menu/getMenuDetail.action?menuId=" + $(parent.document).find("#editMenuId").val();
    $.ajax({
        url: url,
        method: 'get',
    }).then(res => {
        if (res.code === "1") {
            ...
            $("#menuParentName").val(data.parentMenuName);
            if (data.parentMenuName) {
                $(".clear-icon").show();
            }
            $('input[name="menuType"]').filter('[value="' + data.menuType + '"]').prop('checked', true);
            $("#menuName").val(data.menuName);
            $("#url").val(data.url);
            $("#perms").val(data.perms);
            $("#orderNum").val(data.orderNum);
            $("#icon").val(data.icon);
            $('input[name="visible"]').filter('[value="' + data.visible + '"]').prop('checked', true);
            $('input[name="target"]').filter('[value="' + data.target + '"]').prop('checked', true);
        } else {
            $('#btn-edit-menu').remove();
        }
    });
...
function menuTreeClick() {
    ...
    $.ajax({
        type: "GET",
        url: "/menu/getMenuTree.action", // 替换为实际的 API 接口地址
        dataType: "json",
