$(function () {
    ...
    $('.clear-icon').click(function () {
        $('#deptParentName').val('').focus(); // 清空输入框内容并聚焦到输入框
        $(this).hide(); // 隐藏清空图标
    });
    $("#deptId").val($(parent.document).find("#editDeptId").val());
    let url = "/dept/getDeptDetail.action?deptId=" + $(parent.document).find("#editDeptId").val();
    // 点击清空图标时清空输入框内容并隐藏图标
    $('.clear-icon').click(function () {
    ...
    $.ajax({
        ...
        if (res.code === "1") {
            ...
            if (data.deptParentName) {
                $(".clear-icon").show();
            }
            $("#deptName").val(data.deptName);
            $("#deptCode").val(data.deptCode);
            $("#deptLeader").val(data.deptLeader);
            $("#deptPhone").val(data.deptPhone);
            $("#deptEmail").val(data.deptEmail);
            $('input[name="deptStatus"]').filter('[value="' + data.deptStatus + '"]').prop('checked', true);
        } else {
            $('#btn-edit-dept').remove();
        }
    });
